...
pAuraEffectHandler AuraEffectHandler[TOTAL_AURAS]=
{
    ...
    &AuraEffect::HandleModMaxPowerType,                           //418 SPELL_AURA_MOD_MAX_POWER_TYPE
...
void AuraEffect::HandleModMechanicImmunity(AuraApplication const* aurApp, uint8 mode, bool apply) const
{
    ...
    switch (GetId())
    {
        ...
        case 129020:// Avatar
        case 79438: // Soulburn: Demonic Circle
        {
            target->ApplySpellImmune(GetId(), IMMUNITY_MECHANIC, MECHANIC_SNARE, apply);
            target->ApplySpellImmune(GetId(), IMMUNITY_MECHANIC, MECHANIC_ROOT, apply);
            target->ApplySpellImmune(GetId(), IMMUNITY_MECHANIC, MECHANIC_DAZE, apply);
...
void AuraEffect::HandleModMaxPowerType(AuraApplication const* aurApp, uint8 mode, bool apply) const
{
    if (!(mode & (AURA_EFFECT_HANDLE_CHANGE_AMOUNT_MASK | AURA_EFFECT_HANDLE_STAT)))
        return;
    ...
    int32 _new = apply ? target->GetMaxPower(power) + GetAmount() : target->GetMaxPower(power) - GetAmount();
    target->SetMaxPower(power, _new);
}
...
void AuraEffect::HandlePeriodicDamageAurasTick(Unit* target, Unit* caster) const
{
    ...
    if (GetAuraType() == SPELL_AURA_PERIODIC_DAMAGE)
    {
        ...
        // Treant Rake
        else if (GetSpellInfo()->Id == 150017)
        {
            if (caster->GetTypeId() == TYPEID_UNIT)
                if (Unit* _caster = caster->GetCharmerOrOwner())
                    damage += _caster->GetTotalAttackPowerValue(BASE_ATTACK) * 0.24f;
        }
