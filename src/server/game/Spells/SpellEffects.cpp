        ...
        switch (m_spellInfo->SpellFamilyName)
        {
            ...
            case SPELLFAMILY_PALADIN:
            {
                switch (m_spellInfo->Id)
                {
                    ...
                    case 130552:// Word of Glory Damage
                    {
                        if (m_caster->GetTypeId() != TYPEID_PLAYER)
                            return;
            ...
            case SPELLFAMILY_DRUID:
            {
                switch (m_spellInfo->Id)
                {
                    ...
                    case 150017: // Rake - Treant
                    {
                        if (m_caster->GetTypeId() != TYPEID_UNIT)
                            return;

                        if (Unit* _caster = m_caster->GetCharmerOrOwner())
                            damage += int32(_caster->GetTotalAttackPowerValue(BASE_ATTACK) * 0.24f);
                    }
                    default:
                        break;
                }
            ...
            case SPELLFAMILY_HUNTER:
            {
                switch (m_spellInfo->Id)
                {
                    case 13812: // Explosive Trap
                        if (m_caster->GetCharmerOrOwnerOrSelf()->HasAura(119403)) // Glyph of Explosive Trap
                            return;
                        break;
                    case 16827: // Claw
                    case 17253: // Bite
                ...
                case 119905:// Cauterize Master (Command Demon)
                {
                    if (m_caster->GetTypeId() == TYPEID_PLAYER)
                        if (Pet* pet = m_caster->ToPlayer()->GetPet())
                        {
                            ...
                            m_caster->ToPlayer()->AddSpellCooldown(m_spellInfo->Id, 0, pet->GetCreatureSpellCooldownDelay(119899) * 1000, true);
                        }
...
void Spell::EffectEnergizePct(SpellEffIndex effIndex)
{
    ...
    {
        case 123761: // Mana Tea - glyphed
        {
            if (Aura* aura = GetCaster()->GetAura(115867))
            {
                if (aura->GetStackAmount() >= 2)
                {
                    gain *= 2;
                    aura->SetStackAmount(aura->GetStackAmount() - 1); // already removed 1 stack from proc
...
void Spell::EffectOpenLock(SpellEffIndex effIndex)
{
    ...
    if (!m_CastItem && skillId != SKILL_NONE)
    {
        ...
        if (uint32 pureSkillValue = player->GetPureSkillValue(skillId))
        {
            if (gameObjTarget)
            {
                if (gameObjTarget->GetGoType() == GAMEOBJECT_TYPE_CHEST && gameObjTarget->GetGOInfo()->chest.mopMiningMin) // I don't undertand these fields yet
                    reqSkillValue = pureSkillValue;
        ...
        case SUMMON_CATEGORY_UNK:
        {
            ...
            switch (properties->Type)
            {
                ...
                case SUMMON_TYPE_STATUE:
                {
                    ...
                    if (properties->Type == SUMMON_TYPE_LIGHTWELL || properties->Type == SUMMON_TYPE_STATUE)
                    {
                        summon->SetOwnerGUID(m_originalCaster->GetGUID());
                        summon->SetUInt32Value(UNIT_CREATED_BY_SPELL, m_spellInfo->Id);
                    }
                ...
                default:
                {
                    ...
                    if (properties->Type == SUMMON_TYPE_NONE)
                        break;
...
void Spell::EffectKillCreditPersonal(SpellEffIndex effIndex)
{
    ...
    unitTarget->ToPlayer()->KilledMonsterCredit(m_spellInfo->Effects[effIndex].MiscValue, 0, true);
}
    ...
    for (uint32 count = 0; count < numGuardians; ++count)
    {
        ...
        switch (summon->GetEntry())
        {
            ...
            case ENTRY_TREANT_FERAL:
            {
                if (m_targets.GetUnitTarget())
                {
                    summon->CastSpell(m_targets.GetUnitTarget(), 113770, true); // root
                    summon->GetAI()->AttackStart(m_targets.GetUnitTarget());
                }
                break;
            }
            ...
            case ENTRY_TREANT_GUARDIAN:
            {
                if (m_targets.GetUnitTarget())
                {
                    summon->CastSpell(m_targets.GetUnitTarget(), 113830, true); // taunt
                    summon->GetAI()->AttackStart(m_targets.GetUnitTarget());
            ...
            case ENTRY_TREANT_RESTO:
            {
                if (m_targets.GetUnitTarget())
                {
                    summon->CastSpell(m_targets.GetUnitTarget(), 142421, true); // heal
