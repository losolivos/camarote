    ...
    switch (targetType.GetTarget())
    {
        ...
        case TARGET_DEST_CASTER_FRONT:
            if (m_caster->GetTypeId() == TYPEID_UNIT && !m_caster->ToCreature()->isLosDisabled())
                ...
                m_caster->GetFirstCollisionPosition(pos, float(dist - 2.0f), angle);
            break;
...
void Spell::SelectImplicitTargetDestTargets(SpellEffIndex effIndex, SpellImplicitTargetInfo const& targetType)
{
    ...
    if (targetType.GetTarget() == TARGET_DEST_TARGET_BACK) // Shadowstep-like spells
...
SpellMissInfo Spell::DoSpellHitOnUnit(Unit* unit, uint32 effectMask, bool scaleAura)
{
    ...
    if (m_caster != unit)
    {
        ...
        if (m_spellInfo->Speed > 0.0f && unit->HasFlag(UNIT_FIELD_FLAGS, UNIT_FLAG_NON_ATTACKABLE) && unit->GetCharmerOrOwnerGUID() != m_caster->GetGUID() && !IsTriggered())
            return SPELL_MISS_EVADE;
...
SpellCastResult Spell::CheckCast(bool strict)
{
    ...
    if (m_caster->GetTypeId() == TYPEID_PLAYER && !(m_spellInfo->Attributes & SPELL_ATTR0_PASSIVE))
    {
        ...
        if (player->HasSpellCooldown(m_spellInfo->Id) && !player->HasAuraTypeWithAffectMask(SPELL_AURA_ALLOW_CAST_WHILE_IN_COOLDOWN, m_spellInfo) && !IsTriggered())
        {
            if (m_triggeredByAuraSpell)
                return SPELL_FAILED_DONT_REPORT;
            else
